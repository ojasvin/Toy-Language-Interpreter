...
class Errors:
	def eval(raisevalue,raisearg):
		raise raisevalue(raisearg)
		exit()
...
class While:
	...
	def eval(self):
		...
		if 'done' not in self.statements[l-1]:
			Errors.eval(SyntaxError,'done at end of while loop missing')

		if 'done;' not in self.statements[l-1]:
			Errors.eval(SyntaxError,'SemiColon missing at end of done')
...
class IfElse:
	...
	def eval(self):
		condition=self.statements[0]
		l=len(self.statements)
		if 'fi' not in self.statements[l-1]:
			Errors.eval(SyntaxError,'fi at end of if else block missing')

		if 'fi;' not in self.statements[l-1]:
			Errors.eval(SyntaxError,'SemiColon missing at end of fi')
			
		i=condition.find("then")
		condition=condition[len("if"):i]
		if 'else' in self.statements:
			end='else'
		else:
			end='fi;'

		if Expression.eval(condition):
			c=CompoundStatement(self.statements[1:self.statements.index(end)])
			c.eval()
...
class Assignment:
	...
	def eval(self):
		...
		j=self.text.find(";")
		if j==-1:
			Errors.eval(SyntaxError,'SemiColon Missing at end of Assignment Statement')
...
class Var:
	def eval(text):
		if text in state:
			...
			Errors.eval(NameError,'name is not defined')	
...
class Divide():
	def eval(text):
		idx=text.index('/')
		if Expression.eval(text[idx+1:])!=0:
			return(Expression.eval(text[:idx])//Expression.eval(text[idx+1:]))
		else:
			Errors.eval(RuntimeError,'Division by zero')	
